...
export default function TransactionList({ transactions, type, onEdit, onViewDetail }: TransactionListProps) {
  ...
  const Icon = isIncome ? ArrowDown : (isExpense ? ArrowUp : ArrowDown); // Simplistic icon logic for 'all'
  ...
  return (
    <div className="space-y-1">
      {transactions.map((tx) => {
        ...
        const TxIcon = txIsIncome ? ArrowDown : ArrowUp;
        return (
          <div 
            key={tx.id} 
          ...
          >
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <p className="font-bold text-gray-800 dark:text-gray-100 text-sm truncate">{tx.description}</p>
                ...
                {tx.notes && (
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 italic line-clamp-1">{tx.notes}</p>
                )}
              </div>
            </div>
            <div className="flex flex-col items-end gap-2 ml-4 flex-shrink-0">
              <div className={`font-bold text-sm sm:text-base ${txIsIncome ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
